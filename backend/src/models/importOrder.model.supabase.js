...
const ImportOrder = {
  ...
  async create(orderData, userId) {
    const { supplier_id, expected_delivery, delivery_date, notes, items } = orderData;

    // Generate order number if not provided
    const randomSuffix = Math.random().toString(36).substring(2, 8).toUpperCase();
    const order_number = orderData.order_number || `IO-${Date.now()}-${randomSuffix}`;
    ...
    return executeQuery(async () => {
      ...
      const { error: itemsError } = await supabase.from('import_order_items').insert(orderItems);
  ...
  async update(id, orderData) {
    const { supplier_id, expected_delivery, delivery_date, status, notes, items } = orderData;

    return executeQuery(async () => {
      const updateData = {};
      if (supplier_id !== undefined) updateData.supplier_id = supplier_id;
      if (expected_delivery !== undefined) updateData.expected_delivery = expected_delivery;
      ...
      if (items) {
        ...
        await supabase.from('import_order_items').delete().eq('import_order_id', id);
